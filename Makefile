...
jupyter:
	PYTHONPATH=`pwd` screen .venv/bin/jupyter notebook --port=8300 --no-browser .

browser:
	PYTHONPATH=`pwd` screen .venv/bin/jupyter notebook --port=8300 .
...
train: $D/ru_syntagrus-ud-train.json $D/ru_syntagrus-ud-test.json
	OPENBLAS_NUM_THREADS=1 $S train -g 1 -G ru $M $D/ru_syntagrus-ud-train.json $D/ru_syntagrus-ud-test.json
	cp -r $M/* ru2/
